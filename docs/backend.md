...
| | 用处 | 靠不住的地方 |
|---|---|---|
| 标准输出 | 从终端跑时最直接 | 双击启动、被外壳拉起来时根本没有控制台 |
| 内存环形缓冲 | 界面上的控制台页 | 只留 4000 行，进程一退就没了 |
| **文件** | 前两个都没有的时候 | —— |

文件默认在程序旁边的 `logs/dreamtexture.log`，`-log-dir` 可改；那儿写不了
（装到 Program Files 之类）就退到用户缓存目录，而不是干脆不记。超过 8 MB
轮转成 `.log.1`，**只留一代**——日志是查"刚才发生了什么"的，攒一堆
`.1 .2 .3` 只会让人不知道该看哪个。

每次启动写一段分隔与环境信息（版本、系统、程序路径、工作目录、配置路径），
以及配置载入后解析出的绝对路径。一个文件里会攒好几次启动，没有分隔就分不清
哪段是这次的；而那些路径是别人把日志发过来时你第一个想知道的东西。

**启动失败也进文件。** 以前这类错误只往 stderr 打一行就退出，
而没有控制台的时候那一行谁也看不见。
