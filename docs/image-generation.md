...
设置页的「提示词扩写模型」是个下拉框，选项来自 `GET /api/prompts/models`。
它走 `textBase()` / `textAuth()`，**不是**出图那套——两者可以配在不同网关上，
拿出图网关的清单去填扩写的下拉框，填出来的东西一个都用不了。

**筛法和图像模型正好相反：这里用排除法。** 两者错认的代价不对称——

| | 漏认 | 错认 |
|---|---|---|
| 图像（前缀白名单） | 少一个选项 | 点了生成才报错 |
| 文本（排除黑名单） | **我的模型在下拉框里找不到** | 点了扩写才报错 |

对话模型的命名太散（`gpt-*`、`o*`、`chatgpt-*`，还有网关自己接的
deepseek/qwen/claude/glm…），白名单必然漏掉一大片。所以只把明确不是对话的
剔掉（embedding / tts / whisper / image / moderation / rerank…）。

剔错了也不致命：下拉框是 `filterable` + `tag`，可以直接把模型名打进去。
清单只是帮忙，不是限制——网关不提供 `/models`、或令牌只有调用权限没有列举
权限，都很常见，这时列不出来会显示一句说明，手输照样能用。

当前已保存的那个模型一定会出现在选项里，哪怕服务端没列出它——不然下拉框
会显示空白，让人以为设置丢了。
